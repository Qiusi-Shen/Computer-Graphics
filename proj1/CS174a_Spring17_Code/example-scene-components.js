...
Declare_Any_Class( "Bee_Scene",  // An example of drawing a hierarchical object using a "model_transform" matrix and post-multiplication.
  { 'construct'( context )
      { var shapes = { "box" : new        Cube(), 
                       ...
                       "sub_sphere": new Subdivision_Sphere( 5 ) };
        this.submit_shapes( context, shapes );
         ...
         this.define_data_members( { yellow_clay: context.shaders_in_use["Phong_Model"].material( Color(  1,  1, .3, 1 ), .2, 1, .7, 40 ),
                                    ...
                                    purplePlastic: context.shaders_in_use["Phong_Model"].material( Color( .5, .5, .3, 1 ), .2, 1,  1, 40 ),
                                    greyPlastic: context.shaders_in_use["Phong_Model"].material( Color( .5, .5, .3, 1 ), .2, 1,  1, 40 ),
                                    green: context.shaders_in_use["Phong_Model"].material( Color( .3, .9, .3, 1 ), .1, 1,  1, 40 ),
                                    black: context.shaders_in_use["Phong_Model"].material( Color( 0, 0, 0, 1 ), .1, 1,  1, 40 ),
                                    redfire: context.shaders_in_use["Phong_Model"].material( Color( .8, .1, .1, 1 ), .1, 1,  1, 40 ) } );
                                    
      },
      'draw_laten'(model_transform,graphics_state)
      {
        model_transform = mult(model_transform, rotation(50, 1, 0, 0));
        ...
        //model_transform = mult(model_transform, ro)
      },
       'draw_raten'(model_transform,graphics_state)
      {
        model_transform = mult(model_transform, rotation(-50, 1, 0, 0));
        model_transform = mult(model_transform, translation(3,1,0));
        model_transform = mult(model_transform, scale(0.2,0.5,0.2));
      ...
      'draw_tail'(model_transform,graphics_state)
      {
      ...
      'draw_bee'(model_transform,graphics_state)
      {
          ...
          this.shapes.ball.draw(graphics_state, model_transform, this.brown_clay );
          model_transform = mult(model_transform, scale(1/3, 1/2, 1/2));
          model_transform = mult(model_transform, translation(5,0,0));

          this.draw_rwings(model_transform,graphics_state);
          this.draw_lwings(model_transform,graphics_state); 
          this.draw_tail(model_transform,graphics_state); 
          this.draw_laten(model_transform,graphics_state); 
          this.draw_raten(model_transform,graphics_state); 

  
      },

      'draw_rwings'(model_transform,graphics_state)
      {
          var t = graphics_state.animation_time/10;
          model_transform = mult(model_transform, translation(0,1,1));
          model_transform = mult(model_transform, rotation(70*Math.sin(0.01*t), 1, 0, 0));
          model_transform = mult(model_transform, translation(0, 0, 1.5));
          model_transform = mult(model_transform, scale(1, 0.015, 1.5));
          this.shapes.box.draw( graphics_state, model_transform, this.brown_clay);
      },

      'draw_lwings'(model_transform,graphics_state)
      {
          var t = graphics_state.animation_time/10;
          model_transform = mult(model_transform, translation(0,1,-1));
          model_transform = mult(model_transform, rotation(70*Math.sin(-0.01*t), 1, 0, 0));
      ...
      'draw_rleg'(model_transform, graphics_state)
                  {
                  
                  var t = graphics_state.animation_time/1000;

                  model_transform = mult(model_transform, translation(0,0,0.5));
                  model_transform = mult(model_transform,rotation(Math.abs(10*Math.sin(-t)),1,0,0));
                  model_transform = mult(model_transform, translation(-.5,-2,.5));
                  model_transform = mult(model_transform, scale(.1,1,.1));
                  this.shapes.box.draw(graphics_state, model_transform, this.brown_clay);
                  
                  model_transform = mult(model_transform, scale(10,1/1,10));
                  model_transform = mult(model_transform, translation(.5,2,-.5));
                  
                  
                  model_transform = mult(model_transform, translation(-.4,-3,0));
                  model_transform = mult(model_transform,rotation(Math.abs(50*Math.sin(-t)),1,0,0));
                  model_transform = mult(model_transform, translation(-.1,-.6,.5));
                  model_transform = mult(model_transform, scale(.1,1,.1));
                  this.shapes.box.draw(graphics_state, model_transform, this.brown_clay);
                },
                
          'draw_lleg'(model_transform, graphics_state)
                  {
                  ...
                  model_transform = mult(model_transform, translation(-.4,-3,0));
                  model_transform = mult(model_transform,rotation(-Math.abs(50*Math.sin(-t)),1,0,0));
                  model_transform = mult(model_transform, translation(-.1,-0.6,-.5));
                  model_transform = mult(model_transform, scale(.1,1,.1));
                  this.shapes.box.draw(graphics_state, model_transform, this.brown_clay);
                  },
      ...
      'draw_flower'(model_transform,graphics_state)
                  {
                   ...
                   model_transform = mult(model_transform, scale(.01, .5, .01));
                ...
                this.shapes.sub_sphere.draw( graphics_state, model_transform, this.redfire);
                },
    ...
    'display'( graphics_state )
      { 
        ...
        graphics_state.lights = [ new Light( vec4(  30,  30,  34, 1 ), Color( 0, .4, 0, 1 ), 100000 ),      // Arguments to construct a Light(): Light source position or 
           ...
           this.draw_rleg(model_transform,graphics_state);
           this.draw_lleg(model_transform,graphics_state);
            model_transform = mult(model_transform, translation(1,0,0));
            this.draw_rleg(model_transform,graphics_state);
           this.draw_lleg(model_transform,graphics_state);
           model_transform = mult(model_transform, translation(1,0,0));
